...
class YUKLE{
    ...
    */
    function UPLOAD($MY_FILES)
    {
        ...
        $dosyalar = array();
        $dosyaSay = count($MY_FILES['name']);

		// yüklenecek dosya sayısına göre "dosyalar" dizisini oluştur
		if( $dosyaSay===1 && !is_array($MY_FILES['name']) ) {
			$dosyalar['name'][] 	 = $MY_FILES['name'];
			$dosyalar['type'][]	 	 = $MY_FILES['type'];
			$dosyalar['tmp_name'][]  = $MY_FILES['tmp_name'];
			$dosyalar['error'][] 	 = $MY_FILES['error'];
			$dosyalar['size'][] 	 = $MY_FILES['size'];
		} else {
			$dosyalar = $MY_FILES;
		}
